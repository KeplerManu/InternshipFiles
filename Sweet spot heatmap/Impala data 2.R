...
es_payload_tbl <- tbl(impala, in_schema("dwh", "es_payload"))
payload_df <- as.data.frame(es_payload_tbl)
head(es_payload_tbl)

payloaddata <- dplyr::tbl(
  impala,
  dbplyr::in_schema("dwh", "es_payload")
)

sqldf("SELECT COUNT(*) frequency, floor(cycle_duration) cycle_duration, floor(dipper_load_metric) dipper_load_metric
FROM es_payload_tbl
GROUP BY cycle_duration, dipper_load_metric")
