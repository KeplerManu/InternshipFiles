...
#set metrics
machines <- c("LD-1350-2221")
machineName <- machines[1]

start_ts <- "2019/06/14-00:00:00"
end_ts   <- "2019/07/14-00:00:00"
metrics <- c("ld_cycles_payload_tonnes", "ld_cycles_total_duration")
...
ggplot(count_tbl,aes(x=(duration - 0.5),y=(payload - 0.5), fill=freq))+
  ...
  scale_x_continuous(breaks = seq(22,60, by=2), expand = c(0,0))+
  scale_fill_viridis(option = "plasma", name = "Frequency") +
  ggtitle("Sweet Spot") +
  coord_fixed(ylim = c(10,40), xlim = c(22,60))+
